...
class TestAll(unittest.TestCase):
    ...
    @parameterized.expand(itertools.product([2,3,5,10], [21, 21 * 3, 21 * 6], [0, 1]))
    def test_valid_change_n_splits(self, initial_nsplits, threshold_ndates, splitter_idx):
        ...
        so2 = SignalOptimizer(
            inner_splitter=ExpandingKFoldPanelSplit(n_splits=int(mods_df.n_splits_used.max())) if splitter_idx == 0 else RollingKFoldPanelSplit(n_splits=int(mods_df.n_splits_used.max())),
            X=self.X_train,
            y=self.y_train,
        )
        so2.calculate_predictions(
        ...
        num_new_dates = len(pd.bdate_range(mods_df.real_date.min(), mods_df.real_date.max()))
        self.assertEqual(largest_nsplits, initial_nsplits + (num_new_dates // threshold_ndates))
