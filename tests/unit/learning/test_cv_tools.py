...
class TestAll(unittest.TestCase):
    ...
    def test_param_checks(self):
        ...
        # splitter
        with self.assertRaises(TypeError):
            panel_cv_scores(X=self.X, y=self.y, splitter=1, estimators=self.estimators, scoring=self.scoring)
        with self.assertRaises(TypeError):
        ...
        # show_longbias 
        with self.assertRaises(TypeError):
            panel_cv_scores(X=self.X, y=self.y, splitter=self.splitter, estimators=self.estimators, scoring=self.scoring, show_longbias=1)
        # show_std
        with self.assertRaises(TypeError):
            panel_cv_scores(X=self.X, y=self.y, splitter=self.splitter, estimators=self.estimators, scoring=self.scoring, show_std=1) 
        # verbose
        with self.assertRaises(TypeError):
            panel_cv_scores(X=self.X, y=self.y, splitter=self.splitter, estimators=self.estimators, scoring=self.scoring, verbose="hello")
        with self.assertRaises(ValueError):
            panel_cv_scores(X=self.X, y=self.y, splitter=self.splitter, estimators=self.estimators, scoring=self.scoring, verbose=-50)
        # n_jobs
        with self.assertRaises(TypeError):
            panel_cv_scores(X=self.X, y=self.y, splitter=self.splitter, estimators=self.estimators, scoring=self.scoring, n_jobs="hello")

    def test_output(self):
        output = panel_cv_scores(X=self.X, y=self.y, splitter=self.splitter, estimators=self.estimators, scoring=self.scoring)
