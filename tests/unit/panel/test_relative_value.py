...
class TestAll(unittest.TestCase):
    ...
    def dataframe_generator(self):
        ...
        df_cids: pd.DataFrame = pd.DataFrame(index=self.cids,
        ...
        df_xcats: pd.DataFrame = pd.DataFrame(index=self.xcats,
                                columns=['earliest', 'latest', 'mean_add', 'sd_mult',
                                         'ar_coef', 'back_coef'])
    ...
    def test_relative_value_dimensionality(self):
        ...
        with self.assertRaises(AssertionError):
            ...
            dfd_1: pd.DataFrame = make_relative_value(df=dfd, xcats=['GROWTH', 'INFL'], cids=None,
                                        blacklist=None, rel_meth='subtraction',
                                        rel_xcats=None, postfix='RV')

        with self.assertRaises(AssertionError):
            # Validate the assertion on "xcats" parameter.
            dfd_2: pd.DataFrame = make_relative_value(df=dfd, xcats=('XR', 'GROWTH'), cids=None,
        ...
        with self.assertRaises(AssertionError):
            ...
            dfd_3: pd.DataFrame = make_relative_value(df=dfd, xcats=['XR', 'GROWTH'], cids=['GBP'],
        ...
        with self.assertRaises(RuntimeError) as context:
            make_relative_value(df=dfd, xcats=['GROWTH', 'INFL'], cids=['AUD'],
                                blacklist=None, basket=['AUD'],
                                rel_meth='subtract', rel_xcats=None, postfix='RV')
            run_error: str = "Computing the relative value on a single cross-section using a " \
        ...
        dfx: pd.DataFrame = reduce_df(df=self.dfd, xcats=xcats, cids=cids, start=start, end=end,
        ...
        dfd_rl: pd.DataFrame = make_relative_value(df=self.dfd, xcats=xcats, cids=cids, start=start,
        ...
        dfx: pd.DataFrame = reduce_df(df=self.dfd, xcats=[xcats], cids=cids, start=start,
        ...
        dfx: pd.DataFrame = reduce_df(df=dfd, xcats=xcats, cids=cids, start=start, end=end,
                        blacklist=None, out_all=False)
        ...
        dfd_rl: pd.DataFrame = make_relative_value(df=dfdx, xcats=xcats, cids=cids,
                                                    start=start, end=end, blacklist=None,
                                                    basket=None, complete_cross=False,
                                                    rel_meth='subtract', rel_xcats=None,
                                                    postfix='RV')
        ...
        dfd_rl: pd.DataFrame = make_relative_value(df=dfdx, xcats=xcats, cids=cids, start=start,
                                        end=end, blacklist=None, basket=self.cids,
    ...
    def test_relative_value_logic(self):
        ...
        dfd_2: pd.DataFrame = make_relative_value(df=dfd, xcats=["INFL"], cids=self.cids,
        ...
        dfx: pd.DataFrame = reduce_df(df=dfd, xcats=[xcats], cids=self.cids, start=start,
                                        end=end, blacklist=None, out_all=False)

        dfd_3: pd.DataFrame = make_relative_value(df=dfx, xcats=xcats, cids=self.cids,
        ...
        dfd_3_pivot: pd.DataFrame = dfd_3.pivot(index='real_date', columns='cid', values='value')
        ...
        # randomly.
        dfd_4: pd.DataFrame = make_relative_value(df=dfx, xcats=xcats, cids=self.cids,
                                    blacklist=self.blacklist, basket=basket_cid,
                                    rel_meth='divide', rel_xcats=None, postfix='RV')
        ...
        dfd_4_pivot: pd.DataFrame = dfd_4.pivot(index='real_date', columns='cid', values='value')
