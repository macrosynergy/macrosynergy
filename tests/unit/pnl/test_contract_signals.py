...
class TestContractSignals(unittest.TestCase):
    ...
    def test_contract_signal_with_volatility_adjustment(self):
        ...
        p_leverage = 1 / pd.DataFrame([[0.5, 1.0, 2.0]], index=p.index, columns=p.columns.str.split("_").map(lambda x: x[0] + "_FXLEV"))
        ...
        dfc: pd.DataFrame = contract_signals(dfx, sig="SIGNAL", cids=["AUD", "GBP", "EUR"], ctypes=["FX"], cscales=["FXLEV"])

        self.assertIsInstance(dfc, pd.DataFrame)
        for cid in ["AUD", "GBP", "EUR"]:
            self.assertTrue((dfc.loc[dfc.cid == cid, "value"] == p_leverage[f"{cid:s}_FXLEV"].iloc[0]).all())
