...
class TestAll(unittest.TestCase):
    ...
    def test_reduce_df_general(self):
        ...
        dfd_x = reduce_df(final_df, fields_cats, fields_cids, cids_cats, xcats = ['CRY'],
                             cids = cids[0:2], start = '2013-01-01', end = '2019-01-01')
    ...
    def test_reduce_df_intersect(self):
        ...
        dfdx = final_df[filt1]
        dfd_x1, xctx, cidx = reduce_df(dfdx, fields_cats, fields_cids, cids_cats,
                                       xcats = ['XR', 'CRY'], cids = cids, intersect = True, out_all = True)
    
        self.assertTrue(cidx == ['CAD', 'GBP'])

    def test_reduce_df_black(self):
        black = {'AUD_1': ['2011-01-01', '2012-12-31'], 'AUD_2': ['2018-01-01', '2100-01-01']}
        dfd_x = reduce_df(final_df, fields_cats, fields_cids, cids_cats, xcats = ['XR'], cids = cids,
                                       blacklist = black)
        ...
        dfd_x = reduce_df(final_df, fields_cats, fields_cids, cids_cats, xcats = ['XR'], cids = cids, blacklist = black)
    ...
    def test_categories_df_conversion(self):
        dfc = categories_df(final_df, ['XR', 'CRY'], fields_cats, fields_cids, cids_cats,
                            cids, freq = 'M', lag = 0, xcat_aggs = ['mean', 'last'])

        filt1 = (final_df['real_date'].dt.year == 2011) & (final_df['real_date'].dt.month == 10)
        filt2 = (final_df['cid'] == 'AUD') & (final_df['xcat'] == 'XR')
        x1 = final_df[filt1 & filt2]['value'].mean()
        x2 = float(dfc.loc[('AUD', '2011-10-31'), 'XR'])
        self.assertAlmostEqual(x1, x2)  ## Check correct application of monthly mean.

        filt2 = (final_df['cid'] == 'AUD') & (final_df['xcat'] == 'CRY')
        x1 = final_df[filt1 & filt2]['value'].iloc[-1]
    ...
    def test_categories_df_lags(self):
        dfc = categories_df(final_df, ['XR', 'CRY'], fields_cats, fields_cids, cids_cats,
                            cids, freq = 'M', lag = 1, fwin = 3, xcat_aggs = ['mean', 'last'])

        filt1 = (final_df['real_date'].dt.year == 2013) & (final_df['real_date'].dt.month.isin([10, 11, 12]))
        filt2 = (final_df['cid'] == 'AUD') & (final_df['xcat'] == 'XR')
        x1 = round(float(np.mean(final_df[filt1 & filt2].set_index('real_date').resample('M').mean())), 10)
    ...
    def test_categories_df_black(self):
        black = {'CAD': ['2014-01-01', '2014-12-31']}
        dfc = categories_df(final_df, ['XR', 'CRY'], fields_cats, fields_cids, cids_cats,
                            cids = cids, freq = 'M', xcat_aggs = ['mean', 'last'], blacklist = black)
