...
class TestAll(unittest.TestCase):
    ...
    def test_reduce_df_intersect(self):
        ...
        dfd_x1, xctx, cidx = reduce_df(
            dfdx, xcats=["XR", "CRY"], cids=self.cids, intersect=True, out_all=True
        )
    ...
    def test_categories_df_multiple_xcats(self):
        ...
        filt_3 = (
            (df_copy["month"] == fixed_date.month)
            & (df_copy["year"] == fixed_date.year)
            & (df_copy["cid"] == "AUD")
            & (df_copy["xcat"] == ret)
        )
    ...
    def test_categories_df_black(self):
        ...
        self.assertTrue(
            len(
                [
                    item
                    for item in dfc_cad.reset_index()["real_date"]
                    if item in black_range_1
                ]
            )
            == 0
        )
