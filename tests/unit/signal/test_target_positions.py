...
class TestAll(unittest.TestCase):
    ...
    def test_weight_dataframes(self):
        ...
        basket_1 = Basket(df=dfd, contracts=west_contracts, ret="XR_NSA", cry=None)
        ...
        basket_2 = Basket(df=dfd, contracts=apc_contracts, ret="XR_NSA", cry=None)
        basket_2.make_basket(weight_meth="equal", max_weight=0.55, basket_name="APC_EQ")
    ...
    def test_modify_signals(self):
        ...
        df_unit_poz_two = modify_signals(
            df=dfd_copy_two,
            cids=self.cids,
            xcat_sig=xcat_sig,
            start="2012-01-01",
        ...
        # values.
        dfd_sig_w = dfd_signal_copy.pivot(
            index="real_date", columns="cid", values="value"
        )
    ...
    def test_cs_unit_returns(self):
        ...
        with self.assertRaises(AssertionError):
            sigrels_copy = [1, -1, 1]
            df_pos_vt = cs_unit_returns(
                df=dfd, contract_returns=contract_returns, sigrels=sigrels_copy, ret=ret
            )
        ...
        df_pos_vt = cs_unit_returns(
            df=dfd, contract_returns=contract_returns, sigrels=sigrels, ret=ret
        )
        ...
        fx_row = self.row_return(
            dfd=dfd, date=date, c_return=contract_returns[0], sigrel=sigrels[0]
        )
        eq_row = self.row_return(
            dfd=dfd, date=date, c_return=contract_returns[1], sigrel=sigrels[1]
        )
    ...
    def test_basket_handler(self):
        ...
        df_mods = modify_signals(
            df=reduced_dfd, cids=self.cids, xcat_sig=xcat_sig, scale=scale
        )
        ...
        basket_1 = Basket(
            df=reduced_dfd, contracts=apc_contracts, ret="XR_NSA", cry=None
        )
        ...
        df_mods_w_output = basket_handler(
            df_mods_w=df_mods_w, df_c_wgts=dfw_wgs, contracts=apc_contracts
        )
    ...
    def basket_weight(reduced_dfd, df_mods_w, local_contracts, basket_name):
        ...
        basket_1 = Basket(
            df=reduced_dfd, contracts=local_contracts, ret="XR_NSA", cry=None
        )
        basket_1.make_basket(
            weight_meth="invsd",
            lback_meth="ma",
        ...
        df_basket_pos = basket_handler(
            df_mods_w=df_mods_w, df_c_wgts=df_c_wgts, contracts=local_contracts
        )
    ...
    def test_consolidation_help(self):
        ...
        df_basket_pos = self.basket_weight(
            reduced_dfd, df_mods_w, apc_contracts, basket_name="APC_FX"
        )
    ...
    def test_consolidate_positions(self):
        ...
        # baskets.
        df_basket_west = self.basket_weight(
            reduced_dfd, df_mods_w, west_contracts, basket_name="WST_FX"
        )
        df_basket_apc = self.basket_weight(
            reduced_dfd, df_mods_w, apc_contracts, basket_name="APC_EQ"
        )
    ...
    def test_target_positions(self):
        ...
        output_df_vol = target_positions(
            df=reduced_dfd,
            cids=self.cids,
            xcat_sig=xcat_sig,
            ctypes=ctype,
