...
class MultiCredentialDownload:
    ...
    def _download(
        ...
        chunks = [all_expressions[i:i + chunk_size] for i in range(0, len(all_expressions), chunk_size)]

        results = []
        start : float = timer()
        results = Parallel(n_jobs=4)(delayed(JPMaQSDownload(**cred.oauth(mask=False)
                                                            ).download)(expressions=chunk, 
...
if __name__ == "__main__":
    ...
    mcd = MultiCredentialDownload(config=Config())

    x = mcd._download(tickers=tickers, metrics=["all"], start_date="1990-01-01", end_date="2020-01-01",
                    show_progress=True, report_time_taken=True)