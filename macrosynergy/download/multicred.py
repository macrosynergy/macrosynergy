...
class MultiCredentialDownload:
    def __init__(self, config: Config, max_workers: int = 4, save_path: str = None):
        self.config = config
    ...
    def download(
        ...
        results : List[bool] = Parallel(n_jobs=self._max_workers)(
            ...
            for cred, download_args, save_id in zip(creds, download_args_list, save_ids)
        )

        # if any of the results are false, then we need to re-run the failed ones
        if not all(results):
            failed_ids: List[str] = [
                save_id for save_id, result in zip(save_ids, results) if not result
            ]
            ...
            results : List[bool] = Parallel(n_jobs=self._max_workers)(
                ...
                for cred, download_args, save_id in zip(creds, failed_args, failed_ids)
            )

            if not all(results):
                print(f"Failed to download {len(failed_ids)*self.chunk_len} expressions")
