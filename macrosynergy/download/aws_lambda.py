...
API_DELAY_PARAM: float = 0.0  # 300ms delay between requests
...
class AWSLambdaInterface(object):
    """
    High level interface for the AWS Lambda API.

    When using AWS authentication:

    :param <str> access_key_id: access key ID for AWS .
    :param <str> secret_access_key: secret access key for AWS.

    When using AWS credentials file:

    :param <str> cred_file: path to the credentials file.
    ...
    :param <int> batch_size: default 15, number of expressions to send in a single
    ...
    :param <bool> check_connection: whether to send a check_connection request.
        Defaults to True.
    :param <str> base_url: base URL for the AWS Lambda API.
    :param <str> token_url: token URL for the AWS Lambda API.
    :param <bool> suppress_warnings: whether to suppress warnings. Defaults to True.

    :return <AWSLambdaInterface>: AWSLambdaInterface object.
    ...
    def __init__(
        ...
        assert self.auth is not None, (
            ...
            "Check the parameters passed to the AWSLambdaInterface class."
        )
    ...
    def _fetch(
        ...
        """
        Make a request to the Lambda API using the specified parameters.
        ...
        if (response is None) or ("instruments" not in response.keys()):
            ...
            raise InvalidResponseError(
                f"Invalid response from Lambda: {response}\n"
                f"User ID: {self.auth.get_auth()['user_id']}\n"
                f"URL: {form_full_url(url, params)}"
                f"Timestamp (UTC): {datetime.now(timezone.utc).isoformat()}"
            )
        ...
        if "links" in response.keys() and response["links"][1]["next"] is not None:
            logger.debug("Lambda response paginated - get next response page")
    ...
    def get_catalogue(
        ...
        Queries the Lambda API's Groups/Search endpoint to get the list of
        ...
        """
        if verbose:
            print(f"Downloading the {group_id} catalogue from Lambda...")
        try:
    ...
    def _concurrent_loop(
        ...
        **kwargs,
        ...
        """
        Concurrent loop to download data from the Lambda API.
    ...
    def _download(
        ...
        **kwargs,
        ...
        """
        Backend method to download data from the Lambda API.
