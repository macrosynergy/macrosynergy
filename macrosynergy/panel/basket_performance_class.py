...
class Basket(object):
    ...
    def pivot_dataframe(self, tick_list):
        """
        Reduces the dataframe to include a subset of the possible tickers and
        subsequently returns a wide dataframe: each column corresponds to a ticker.

        :param <List[str]> tick_list: list of the respective tickers.

        :param <pd.DataFrame> dfw: wide dataframe.
        """
    ...
    def carry_list(self, cry):
        ...
        :param <pd.DataFrame or List[pd.DataFrame]>
        """
        if isinstance(cry, list):
            dfw_cry = []
            for i in range(self.no_cry):
                dfw_cry.append(self.pivot_dateframe(self.ticks_cry[i]))
        elif isinstance(cry, str):
            dfw_cry = self.pivot_dateframe(self.ticks_cry)
        else:
            dfw_cry = None
