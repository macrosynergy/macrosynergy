...
def linear_composite_on_xcat(
    ...
    # Downsample the weights to the update frequency, but keep the same indexing as the target.
    weights_df = (
        weights_df.resample(update_freq).first().reindex(target_df.index).bfill()
    )
