...
def metric_scatterplot(
    ...
    """
    Scatter one metric against another across configurations

    Parameters
    ----------
    data : pd.DataFrame
        One row per configuration, holding at least the columns named by x and y.
    x : str
        Column of data plotted on the x-axis.
    y : str
        Column of data plotted on the y-axis.
    hue : Optional[str]
        Column of data encoded as the colour of the points.
    style : Optional[str]
        Column of data encoded as the marker shape of the points.
    title : str
        Title of the plot. Defaults to an empty string.
    xlabel : Optional[str]
    ...
    ylabel : Optional[str]
        ...
        column is used.
    trendline : bool
        Whether to fit and draw a trend line through the points. Defaults to True.
    trend_degree : int
        Degree of the polynomial fitted for the trend line. Defaults to 1.
    trend_label : Optional[str]
        Legend label of the trend line. Defaults to None.
    trend_margin : float
        Fraction of the observed range of x-values by which the trend line
        extends beyond the outermost points. Defaults to 0.05.
    title_fontsize : int
    ...
    with sns.axes_style("whitegrid"), sns.plotting_context("notebook"):
        ...
        ax.grid(axis="both", linestyle="--", linewidth=0.6, alpha=0.7)
        ax.set_axisbelow(True)
