...
def impute_panel(df, xcats: list, threshold: float = 0.5):
    """
    ...
    full_idx = pd.MultiIndex.from_frame(
        pd.concat([
            pd.MultiIndex.from_product(
                [complete_df.index.levels[0], complete_df.index.levels[1], pd.Series([xcat], name="xcat")]
    ...
    incomplete_df = complete_df.loc[complete_df["xcat"].isin(xcats), :]
    # computing the data availability stats
    incomplete_df["mean_val"] = incomplete_df.groupby(["xcat", "real_date"])["value"].transform("mean")
    incomplete_df["tot"] = incomplete_df.groupby(["xcat", "real_date"])["value"].transform("size")
    incomplete_df["avail"] = incomplete_df.groupby(["xcat", "real_date"])["value"].transform("count")
    
    # Filling CID-specific values only for the appropriate cids and conditional on representative sample
    mask = (
        incomplete_df["avail"].div(incomplete_df["tot"]) > threshold
    ) & (
        incomplete_df["value"].isna()
    )
    incomplete_df.loc[mask, "value"] = incomplete_df.loc[mask, "value"].fillna(incomplete_df.loc[mask, "mean_val"])
    
    # integrating the new values into the old one
    complete_df = update_df(complete_df, incomplete_df.loc[:, complete_df.columns])
