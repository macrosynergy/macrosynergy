...
class AdaptiveSignalHandler:
    def __init__(
        ...
        """
        Class to create a quantamental trading signal dataframe from the predictions of a
        machine learning model with adaptive hyperparameter selection at each test time and
        analyse the subsequent signals.

        :param <Dict[str, Union[BaseEstimator,Pipeline]]> models: dictionary of sklearn predictors.
        :param <Dict[str, Callable]> metrics: dictionary of metrics to optimise each model over.
    ...
    def create_qdf(
        ...
        """
        Method to create a quantamental dataframe from the predictions of an adaptive machine learning model
        specified by both the class attributes and hyperparameter search specification.
        ...
        :param <Dict[str, Dict[str, List]]> hparam_grid: Nested dictionary denoting the
            ...
            representing the model name and should match with the model keys in the models
            dictionary. The nature of the inner dictionary depends on the hyperparameter search type.
            ...
            If hparam_type is "random", then the inner dictionary should have keys
            corresponding to the hyperparameter names and values either equal to a distribution
            from which to sample or a list of them. The function must have an rvs method. For example:
            hparam_grid = {
...
def adaptive_preds_to_signal(
    ...
    hyperparameter-adaptive model. For each training and test set pair generated by `outer_splitter`,
    a hyperparameter search is run over the training set.
    The model is optimised for each provided metric and the test set predictions are recorded.
    Each test set sample must only appear once in the walk-forward validation. TODO: add check for this.
    ...
    for train_idx, test_idx in tqdm(outer_splitter.split(X, y)):
        ...
        for name, model in models.items():
            ...
            if hparam_grid[name] == {}:
                ...
                model.fit(X_train_i, y_train_i)
                preds: np.ndarray = model.predict(X_test_i)
                collected_data.append([name, test_xs_levels, test_date_levels, preds])
            else:
                # Then there are hyperparameters to search over
                # There must be some metrics specified
                if len(metrics) == 0:
                ...
                for metric in metrics.keys():
                    ...
                    preds: np.ndarray = search_object.predict(X_test_i)
                    collected_data.append(
                        [f"{name}_{metric}", test_xs_levels, test_date_levels, preds]
                    )
...
if __name__ == "__main__":
    ...
    dfd2 = msm.categories_df(df=dfd2,xcats=xcats,cids=cids,val="value",blacklist=black,freq="M")
    ...
    # (1) Example usage for adaptive_preds_to_signal() when no hyperparameters are to be searched over.
    models = {
    ...
    signal_df = AdaptiveSignalHandler(
        ...
        X=X,
        y=y,
    ).create_qdf(hparam_grid={"ols" : {}, "map_ols" : {}}, hparam_type="grid")
