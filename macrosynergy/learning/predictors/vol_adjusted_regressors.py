...
class BaseVolAdjRegressor(BaseEstimator, RegressorMixin, ABC):
    def __init__(
        self,
    ...
    def fit(
        ...
        """
        Fit method to fit the underlying model, as passed into the constructor,
        and subsequently modify predictions based on estimated prediction errors to produce
        a trading signal that accounts for the statistical precision of the predictions.

        :param <pd.DataFrame> X: Pandas dataframe of input features.
        :param <Union[pd.DataFrame, pd.Series]> y: Pandas series or dataframe of targets
            associated with each sample in X.

        :return <BaseVolAdjRegressor>
        """
        # Checks
    ...
    def check_init_params(
        ...
        :param <float> error_offset: A small offset to add to the prediction errors to
            prevent division by zero in the case of very small prediction errors.
        :param <str> bootstrap_method: The bootstrap method used to modify the
        predictions. Accepted values are "panel", "period", "cross", "cross_per_period"
            and "period_per_cross".
        :param <int> bootstrap_iters: The number of bootstrap iterations to perform in
            order to determine the prediction errors of the model parameters under the
            bootstrap approach.
        :param <Union[float, int]> resample_ratio: The ratio of resampling units comprised
            in each bootstrap dataset. This is a fraction of the quantity of the panel
