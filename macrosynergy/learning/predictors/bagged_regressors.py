...
class BaseBaggedPanelRegressor(BaseEstimator, RegressorMixin):
    def __init__(
        self,
        ...
        sample_size_ratio: Optional[Union[float, int]] = None,
    ):
        """
        Base class for regressors with bootstrap aggregation over a panel. 

        :param <RegressorMixin> model: The underlying regression model for which a new
            bagged model will be created.
        :param <int> n_estimators: The number of models in the bagging ensemble. Default
            is 100.  
        :param <str> bootstrap_method: The bootstrap method for the bagging procedure.
            Accepted values are "standard", "bs_panel", "bs_periods", "bs_cross",
            "bs_cross_per_period" and "bs_period_per_cross".
        :param <Optional[Union[str, float, int]]> max_features: The number of features
            to consider when looking for the best split. Default is None, which means
            that all features will be considered. If int, then consider only 
            `max_features` features per model. If float, then `max_features` is a fraction 
            and `int(max_features * n_features)` features are considered per model, unless
            this quantity is greater than one, in which case all features are considered.
            String values of `sqrt` and `log2` are also accepted, in which case
            `int(sqrt(n_features))` and `int(log2(n_features))` features are considered
            per model, respectively. Default is None.
        :param <Union[float, int]> sample_size_ratio: The ratio of the sample size to be
            used in the bagging procedure for training each model. Default is None. 
    ...
    def check_init_params(
        ...
        :param <RegressorMixin> model: The underlying linear model used to construct
            a bagging ensemble.
        :param <int> n_estimators: The number of models in the bagging ensemble.
        :param <str> bootstrap_method: The method used to modify the coefficients.
            Accepted values are "standard", "bs_panel", "bs_periods", "bs_cross",
            "bs_cross_per_period" and "bs_period_per_cross".
        :param <Optional[Union[str, float, int]]> max_features: The number of bootstrap
            iterations to perform in order to determine the standard errors of the model
            parameters.
        :param <Union[float, int]> sample_size_ratio: The ratio of the sample size to be
