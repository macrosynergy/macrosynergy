...
df_cids.loc["GBP"] = ["2019-01-01", "2020-12-31", 0, 1]
...
class MultiTaskFeatureLearning(nn.Module):
    """
    PyTorch model to map input features to a collection of smaller
    latent features, with asset-specific linear heads.
    ...
    head_cids: List[str]
        List of assets with linear heads at training time
    """
    def __init__(self, init_dim, hidden_dim, head_cids):
        super().__init__()
        self.init_dim = init_dim
        self.hidden_dim = hidden_dim
        self.head_cids = head_cids

        self.encoder = nn.Linear(init_dim, hidden_dim, bias = False) # TODO: should a bias be here?
    ...
    def forward(self, x, cid = None):
        ...
        if cid == None:
            ...
            outputs = [
                self.heads[cid](x_embedding)
                for cid in self.head_cids
            ]
            outputs = torch.stack(outputs, dim=0)
            preds = torch.mean(outputs, dim=0)
        else:
            preds = self.heads[cid](x_embedding)
...
class SupervisedTorchModel(BaseEstimator):
    def create_train_valid_splits_(self, X, y):
        ...
        train_heads = self.get_train_heads_(X_train) # Not all cross-sections should have heads, depends on availability
        ...
        # Needed for quick data access during training and evaluation
    ...
    def fit_(
        ...
        for epoch in range(epochs):
            model.train()
            # NOTE: Update each batch or after all batches? That is the question.
            for cid in train_cids:
                optim.zero_grad()
                # Get country batch
                ...
                # Forward pass
                if cid in model.head_cids:
                    y_pred = model(X_batch, cid = cid)
                else:
                    y_pred = model(X_batch)
                # Evaluate loss
                if decoder_sparsity == 0:
                    loss = loss_func(y_batch, y_pred)
                else:
                    loss = loss_func(y_batch, y_pred) + decoder_sparsity * self.decoder_shrinkage(model.heads)
                # Backward pass
                loss.backward()
                optim.step()
    ...
    def get_train_heads_(self, X_train):
        cid_sizes = X_train.groupby(level=0).size()
        filtered_cids = cid_sizes.index[cid_sizes >= self.min_xs_samples].to_numpy()
...
class MTFLRegressor(SupervisedTorchModel, RegressorMixin):
    ...
    def fit(self, X, y):
        # Create training and validation sets for early stopping
        (
            X_train,
            y_train,
            X_valid,
        ...
        # First scale y_train to have mean 0 and std 1, and store training mean and std.
        # NOTE: You assume X is already standardized - will need StandardScaler() in the pipeline
        self.y_train_mean_ = y_train.mean()
        self.y_train_std_ = y_train.std()

        y_train = (y_train - self.y_train_mean_) / self.y_train_std_

        # Second scale y_valid using training mean and std
        y_valid = (y_valid - self.y_train_mean_) / self.y_train_std_
        ...
        self.model = MultiTaskFeatureLearning(
            init_dim = X_train.shape[1],
            hidden_dim = self.hidden_dim,
            head_cids = self.train_heads,
        )
    ...
    def predict(self, X):
        ...
        # Get indexes for each cross-section in X
        test_cids = X.index.get_level_values(0).unique()
        test_cids_samples = {
            cid : torch.Tensor(np.where(X.index.get_level_values(0) == cid)[0]).int()
            for cid in test_cids
        }
        
        # Get predictions for each cid
        with torch.no_grad(): 
            preds = []
            for cid in test_cids:
                if cid in self.train_heads:
                    y_cid_pred = self.model(X_tensor[test_cids_samples[cid]],cid=cid)
                else:
                    y_cid_pred = self.model(X_tensor[test_cids_samples[cid]],cid=None)
                preds.append(y_cid_pred)

            # Stack preds 
            preds = torch.vstack(preds).numpy()

        return preds * self.y_train_std_ + self.y_train_mean_
...
if __name__ == "__main__":
    ...
    X_embeddings = mtfl.transform(X)
    print(X_embeddings)

    preds = mtfl.predict(X)
    print(preds)